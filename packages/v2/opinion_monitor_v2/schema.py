"""Stable input/output contract for the fixed v2.2 taxonomy."""
...
TYPE_BY_STATUS: dict[str, frozenset[str]] = {
    "normal": frozenset({"support"}),
    "attention": frozenset({"neutral", "information_gap", "consultation"}),
...
def validate_output(value: Any) -> ClassificationResult:
    ...
    if not isinstance(subtype, str):
        raise ClassificationError(f"type must be a string, got {type(subtype).__name__}")
