...
MARKER = "PROMOTION_WEEK_KS_COMMENT_REGION_RESTORE_V1"
...
def patch_client(root: Path) -> None:
    ...
    root_new = root_anchor + f'            _ks_enrich_comment_regions(comments_res, comments)  # {MARKER}\n'
    ...
    sub_new = sub_anchor + f'                _ks_enrich_comment_regions(comments_res, sub_comments)  # {MARKER}\n'
...
def check(root: Path) -> dict:
    ...
    try:
        ...
        result["root_enrichment"] = "_ks_enrich_comment_regions(comments_res, comments)" in text
        result["sub_enrichment"] = "_ks_enrich_comment_regions(comments_res, sub_comments)" in text
        result["ok"] = all([
