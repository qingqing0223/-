...
def _chord_down() -> bool:
    return _key_down(VK_LCTRL) and _key_down(VK_LSHIFT)


def _wait_chord_release() -> None:
    while _chord_down():
        time.sleep(0.03)
...
def _capture_point(label: str, rect: tuple[int, int, int, int]) -> tuple[int, int]:
    print()
    print(label)
    print("把鼠标移动到目标位置，然后同时按住【左 Ctrl + 左 Shift】约半秒；按 Esc 可退出。")
    print("无需回到 PowerShell，也不要点击鼠标。程序正在等待组合键 ...", flush=True)
    ...
    while True:
        ...
        if _chord_down():
            ...
            if not (left <= point[0] <= right and top <= point[1] <= bottom):
                print("这个点不在检测到的微信/视频号窗口内，请重新移动鼠标并按组合键。", flush=True)
                continue
            try:
                import winsound
...
def main() -> None:
    ...
    print("请把微信/视频号窗口保持可见。")
    print("部分笔记本的 F8 是音量/媒体键，因此本版改用【左 Ctrl + 左 Shift】记录鼠标位置。")

    search = _capture_point(
        "步骤1/3：把鼠标移到【视频号搜索输入框中间】。",
        rect,
    )
    result_tl = _capture_point(
        "步骤2/3：把鼠标移到【搜索结果列表区域左上角】。尽量避开左侧聊天栏。",
        rect,
    )
    result_br = _capture_point(
        "步骤3/3：把鼠标移到【搜索结果列表区域右下角】。",
        rect,
    )
